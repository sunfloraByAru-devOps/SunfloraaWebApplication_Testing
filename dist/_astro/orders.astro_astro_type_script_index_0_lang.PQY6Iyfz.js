import{supabase as n}from"./supabase.D2dx7TGg.js";function i(t){return"₹"+(Number(t)||0).toFixed(2)}try{const{data:t}=await n.auth.getSession();if(t.session){const{data:e,error:r}=await n.from("orders").select("id, order_number, status, payment_status, total, created_at").order("created_at",{ascending:!1});r&&console.error("[Sunflora] Could not load orders:",r);const a=document.getElementById("orders-list");if(a)if(!e||e.length===0){const o=document.getElementById("orders-empty");o&&o.classList.remove("hidden")}else{const o=t.session.user.email;a.innerHTML=e.map(s=>`
