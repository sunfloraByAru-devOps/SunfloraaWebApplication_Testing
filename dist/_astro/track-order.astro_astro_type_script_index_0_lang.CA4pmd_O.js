import{supabase as c}from"./supabase.D2dx7TGg.js";const r=new URLSearchParams(window.location.search);r.get("order")&&(document.getElementById("order-number-input").value=r.get("order"));r.get("email")&&(document.getElementById("email-input").value=r.get("email"));function m(e){return"₹"+(Number(e)||0).toFixed(2)}function b(e){const o=document.getElementById("track-result");o.classList.remove("hidden");const n=Math.max(0,Math.min(100,e.progress_step/e.total_steps*100)),l=(e.history||[]).map(t=>`
        ...
        ${l?`<div><p class="text-sm font-medium text-[#5C3D2E] mb-2">History</p><ul class="flex flex-col gap-2">${l}</ul></div>`:""}
      `,document.getElementById("retry-payment-btn")?.addEventListener("click",async t=>{t.target.disabled=!0,t.target.textContent="Redirecting...";const{data:a,error:p}=await c.functions.invoke("paytm-initiate",{body:{order_id:e.order_id}});if(p||a?.error){alert(a?.error||"Could not start payment. Please try again."),t.target.disabled=!1,t.target.textContent="Retry Payment";return}const i=document.createElement("form");i.method="POST",i.action=a.action_url,Object.entries(a.params).forEach(([g,y])=>{const d=document.createElement("input");d.type="hidden",d.name=g,d.value=String(y),i.appendChild(d)}),document.body.appendChild(i),i.submit()})}async function u(e,o){const n=document.getElementById("track-error"),l=document.getElementById("track-result");n.classList.add("hidden"),l.classList.add("hidden");const{data:s,error:t}=await c.functions.invoke("track-order",{body:{order_number:e,email:o}});if(t||s?.error){n.textContent=s?.error||"We couldn't find that order.",n.classList.remove("hidden");return}b(s)}document.getElementById("track-form").addEventListener("submit",e=>{e.preventDefault(),u(document.getElementById("order-number-input").value.trim(),document.getElementById("email-input").value.trim())});r.get("order")&&r.get("email")&&u(r.get("order"),r.get("email"));
